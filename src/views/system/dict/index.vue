...
const confirmEvent = async () => {
  const type = await VXETable.modal.confirm("Are you sure you want to delete it?");
  (await type) === "confirm" &&
...
const submitEvent = () => {
  ...
  setTimeout(() => {
    ...
    if (dictData.selectRow) {
      VXETable.modal.message({ content: "Successfully saved", status: "success" });
      Object.assign(dictData.selectRow, dictData.formData);
    } else {
      VXETable.modal.message({ content: "Added successfully", status: "success" });
...
<template>
  <div>
    ...
    <Config :drawer="drawer" drawTitle="list of dictionaries" @handleClose="handleClose" />
  </div>
</template>
